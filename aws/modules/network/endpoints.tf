...
resource "aws_vpc_endpoint" "ssm" {
  ...
  vpc_id            = aws_vpc.vpc.id
  service_name      = format("com.amazonaws.%s.ssm", data.aws_region.current.id)
...
resource "aws_vpc_endpoint" "ssmmessages" {
  ...
  vpc_id              = aws_vpc.vpc.id
  service_name        = format("com.amazonaws.%s.ssmmessages", data.aws_region.current.id)
  vpc_endpoint_type   = "Interface"
  security_group_ids  = [aws_security_group.vpc_endpoint_sg.id]
  subnet_ids          = aws_subnet.private_subnets.*.id
  private_dns_enabled = true
...
resource "aws_vpc_endpoint" "ec2messages" {
  ...
  vpc_id              = aws_vpc.vpc.id
  service_name        = format("com.amazonaws.%s.ec2messages", data.aws_region.current.id)
  vpc_endpoint_type   = "Interface"
  security_group_ids  = [aws_security_group.vpc_endpoint_sg.id]
  subnet_ids          = aws_subnet.private_subnets.*.id
  private_dns_enabled = true
...
resource "aws_vpc_endpoint" "ec2" {
  ...
  vpc_id              = aws_vpc.vpc.id
  service_name        = format("com.amazonaws.%s.ec2", data.aws_region.current.id)
  vpc_endpoint_type   = "Interface"
  security_group_ids  = [aws_security_group.vpc_endpoint_sg.id]
  subnet_ids          = aws_subnet.private_subnets.*.id
  private_dns_enabled = true
...
resource "aws_vpc_endpoint" "monitoring" {
  ...
  vpc_id              = aws_vpc.vpc.id
  service_name        = format("com.amazonaws.%s.monitoring", data.aws_region.current.id)
  vpc_endpoint_type   = "Interface"
  security_group_ids  = [aws_security_group.vpc_endpoint_sg.id]
  subnet_ids          = aws_subnet.private_subnets.*.id
  private_dns_enabled = true
